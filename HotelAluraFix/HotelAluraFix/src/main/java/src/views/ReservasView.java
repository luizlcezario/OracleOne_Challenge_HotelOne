...
public class ReservasView extends JFrame {
	...
	static private RegisterController rgController = RegisterController.getInstance();
	...
	public ReservasView() {
		...
		lblValorSimbolo.setBounds(121, 332, 500, 25);
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				Double valor = calcularValor(txtDataE, txtDataS);
				if (valor != -1) {
		...
		txtFormaPagamento.setModel(new DefaultComboBoxModel(PaymentTypes.getListOfString().toArray()));
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				Double value = calcularValor(ReservasView.txtDataE, ReservasView.txtDataS);
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null && value > 0) {
					Optional<PaymentTypes> pt = PaymentTypes.fromString(ReservasView.txtFormaPagamento.getSelectedItem().toString());
					if (pt.isPresent()) {
						rgController.setNewReserve(value, new Date(ReservasView.txtDataE.getCalendar().getTimeInMillis()),new Date(ReservasView.txtDataS.getCalendar().getTimeInMillis()), pt.get());
						RegistroHospede registro = new RegistroHospede();
						registro.setVisible(true);
					} else {
						JOptionPane.showMessageDialog(null, "Deve preencher todos os com opcoes validas.");
					}
				} else {
	...
	private Double calcularValor(JDateChooser dataE,JDateChooser dataS) {
